    ...
    case "facebook":
      return (
        <svg viewBox="0 0 16 16" fill="none" className={className}>
          ...
          <path
            d="M9.2 5.5H8.3c-.7 0-1 .35-1 1v1.1H9l-.2 1.5H7.3V13H5.8V9.1H4.8V7.6h1V6.3c0-1.15.65-1.9 1.9-1.9h1.5v1.1z"
            fill="currentColor"
          />
        </svg>
      );
  ...
  useEffect(() => {
    ...
    let raf = requestAnimationFrame(function tick() {
      for (let i = 0; i < itemPositionEls.current.length; i++) {
        ...
        const dx =
          mousePos.current.x - (rect.left + rect.width / 2);

        const dy =
          mousePos.current.y - (rect.top + rect.height / 2);
        ...
        if (distance < REPEL_RADIUS) {
          ...
          const force =
            (1 - distance / REPEL_RADIUS) * REPEL_STRENGTH;
  ...
  return (
    ...
    >
      {CLOSE_CIRCLE.map((entry, i) => {
        const angleRad =
          (angleFor(i, CLOSE_CIRCLE.length) * Math.PI) / 180;
              ...
              <motion.div
                ...
                style={{
                  animation: reducedMotion
                    ? undefined
                    : `social-moon-drift 9s ease-in-out infinite`,
                  ...
                  >
                    <span
                      ...
                      style={{
                        background: "var(--surface-2)",
                        boxShadow:
                          "0 0 14px rgba(124,111,238,0.45)",
                      }}
                    >
                      <PlatformGlyph
                        icon={entry.icon}
